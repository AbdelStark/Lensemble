...
> **Current runtime status (2026-07-24 — privacy integration incomplete).** Participant-side
> `local_round()` applies configured clipping and Gaussian noise, but the Phase 3 coordinator accepts
> unmasked per-participant deltas. `Coordinator.try_round()` commits the outer step from those deltas
> before the service runs its simulated/TEE secure-sum report. That sum is a post-commit cross-check, not
> the optimizer input. The same report constructs a fresh DP accountant for each closed round, accounts
> one step, and discards it; there is no cumulative pre-release budget check or run-level hard stop.
> In addition, participant Gaussian noise is deterministically derived from the shared root seed, round,
> and participant id. The coordinator has that config and the root seed is recorded in run manifests, so
> the noise is reconstructible and subtractable by that observer. Participant-secret CSPRNG entropy is
> not yet integrated.
> Consequently, the individual-update-confidentiality and cumulative-$(\varepsilon,\delta)$ guarantees
> specified below are target contracts, not current end-to-end runtime guarantees; the current
> deterministic clip/noise path is mechanism plumbing, not claim-grade DP. Historical Phase 3
> `secure_sum`, `DP-accounted rounds`, and per-round epsilon fields must be read under this boundary.

Current Phase 3 dataflow:

```text
unmasked per-participant deltas -> Coordinator.try_round() -> committed outer step
                              \-> after commit: secure-sum equivalence report
                                               + fresh one-step DP accountant

participant noise seed = derive(shared manifest root seed, round, participant)
```
...
The table states the intended protocol trust model. The current coordinator does not yet satisfy the
secure-aggregator row because it receives individual unmasked deltas.
...
| Public verifier | Untrusted; given only public inputs | Verifies STARK + recomputes the committed-model alignment diagnostic; does not infer an honest participant-side backstop | Public probe $\mathcal{P}$, committed model hashes, dataset roots $R_c$ |

Under the intended protocol, the coordinator is **untrusted with respect to raw data in every phase**;
raw-data residency does not depend on coordinator honesty. Individual-update confidentiality additionally
depends on completing the secure-aggregation integration described above. The coordinator's *honesty
about the aggregation arithmetic* is assumed in Phase 1 and made verifiable in Phase 2.
...
- **Membership-inference and model-inversion attacks on the released global model** beyond the
  protection the intended differential-privacy contract would provide. That contract bounds
  per-participant-round leakage but does not make the model attack-proof; the current deterministic
  mechanism does not satisfy it (§5).
...
leaves; model deltas, dataset commitments, and shared coordination state cross. The following diagram
shows the **target protocol**, not the current coordinator dataflow:
...
In the target protocol, inside participant $c$'s boundary, raw $(o_t, a_t, o_{t+1})$ trajectories drive local
...
The table specifies every intended boundary-crossing message, its direction, contents, and protection.
This reproduces and annotates the protocol message summary
...
| `Update` | participant → aggregator | $\Delta_c$ (flat delta + L2 norm + bound `dataset_root`) | DP (clip+noise, §5) then secure-aggregation mask (§4) | `INV-DP-BOUND`, `INV-RESIDENCY` | `PrivacyBudgetExceeded`, `SecureAggregationError`, `ResidencyViolation` |
...
contract `INV-RESIDENCY` enforces and §3 below details. The `Update` message carries a
`dataset_root` field equal to $R_c$ so the aggregator can bind the contribution without learning the
data (`INV-COMMIT-BINDING`).

The current `Phase3CoordinatorService` does not satisfy the `Update` row's secure-aggregation protection:
it submits an unmasked `PseudoGradient` to the in-process transport and the coordinator consumes that
object directly. Participant-side clip/noise is live, but a `PrivacyBudgetExceeded` pre-release gate is
not.
...
The pseudo-gradient itself is a function of private data and therefore a leakage surface. Under the
target integration, participant-secret differential privacy (§5) bounds that residual leakage and
secure aggregation (§4) hides the individual delta in the aggregate. In the current runtime, the
coordinator receives unmasked deltas and can reconstruct the deterministic noise, so neither target
privacy statement applies. Residency enforcement still guarantees that *raw data and embeddings* do
not leave.

## 4. Secure-aggregation guarantee (`INV-AGG-DETERMINISM` interaction)

The target contract requires the coordinator to learn only the sum $\sum_c \Delta_c$, never an
individual $\Delta_c$, because an individual update leaks more about a silo's data than the sum does
...
the target guarantee, its assumptions, and its interactions. The live coordinator's plaintext commit
path does not implement it.

**Target guarantee.** Under the pairwise-mask (Bonawitz-style) protocol or a TEE-based aggregator, the
...
- With gauge correction: participant-specific Procrustes alignment and full-weight re-difference happen
  before clipping, noise, quantization, encoding, and masking (`INV-ALIGN-BEFORE-RELEASE`). A sum-only
  aggregator cannot apply distinct transforms after reveal; a trusted/MPC alternative must execute the
  same order inside its boundary.
- With DP (§5): per-participant noise is added *before* masking, so the revealed sum carries the summed
...
**Target failure modes.** Dropout below threshold → `SecureAggregationError` (the round retries with the
...
$\Delta_c$ is rejected by design (it leaks individual updates; RFC-0011 Alternatives). The current
plaintext commit path is an integration blocker, not evidence that these failure modes are enforced.

## 5. Differential-privacy guarantee and its limits

The intended differential-privacy contract bounds how much a single participant's contribution to a
round can be inferred from the released (summed) update. The mechanism and accounting are specified in
...
**Mechanism contract.** Per participant, after any required alignment/re-difference and before release:
clip $\Delta'_c \leftarrow \Delta'_c \cdot
\min(1, C_{\text{clip}}/\lVert\Delta'_c\rVert)$ then add
...
The clip/noise operation is live, but its Gaussian generator is derived from
`(config.determinism.root_seed, round_index, participant_id)`. The coordinator has the shared config,
and both generic and Phase 3 run manifests record the root seed. An observer with that seed and matching
runtime can replay and subtract the noise. Claim-grade DP therefore requires participant-secret CSPRNG
entropy that never enters shared config, messages, reports, or manifests. Deterministic seed injection
is retained only for tests and explicitly non-private replay evidence.

**Invariant.** `INV-DP-BOUND` ([conventions §7](conventions.md#7-named-invariants)): after clipping and before noising,
$\lVert\Delta'_c\rVert \le C_{\text{clip}}$. Enforced in `lensemble.privacy.dp`; a violation (clipping
...
**Accounting contract.** `lensemble.privacy.accountant` provides stateful cumulative
$(\varepsilon,\delta)$ accounting over rounds (Rényi-DP / PRV). The required runtime checks the budget
before release and stops with `PrivacyBudgetExceeded` when the next release would exceed it
([conventions §6](conventions.md#6-error-taxonomy)). The live Phase 3 service instead constructs a fresh
accountant after commit, consumes one step, reports `rounds_accounted=1`, and discards it. It therefore
neither composes loss across rounds nor enforces a run-level stop.
...
global-model hash) — the audit substrate (RFC-0004 §5). This gives **round-level provenance** (which
...
### 6.1 Public recomputation of committed-model alignment

Alignment of a **committed global model** on the public probe is a deterministic public diagnostic.
The `recompute_alignment` surface ([conventions §5](conventions.md#5-public-api-surface)) verifies the
checkpoint and probe hashes, recomputes the model's $Q^\star$/residual, and can compare that diagnostic
with a published claim
([RFC-0006 §4](../rfcs/RFC-0006-verifiable-contribution.md#4-public-recomputation-phase-1-free-in-scope-now)).

That does **not** verify the Layer-3 action taken for each participant. The target backstop depends on
each participant's raw local full weights and must execute before clipping, noise, quantization,
encoding, and masking (`INV-ALIGN-BEFORE-RELEASE`;
[RFC-0002 §5](../rfcs/RFC-0002-gauge-and-aggregation.md#5-layer-3--procrustes-re-alignment-at-aggregation-backstop)).
After genuine secure aggregation, the coordinator has only a sum and cannot reconstruct an individual
$Q_c^\star$ or $\Delta'_c$. Participant-side execution therefore remains assumed unless it is attested
or proven, or performed inside a trusted/MPC boundary. Public committed-model recomputation is useful
diagnostic evidence, not proof of that participant-specific pre-release transform.
...
- **Target key management for secure-aggregation masks / TEE.** Once the masking path is integrated,
  pairwise-mask key agreement and threshold shares are held only inside a participant process and never
  leave its boundary (they are not
...
[RFC-0006](../rfcs/RFC-0006-verifiable-contribution.md) specifies cryptographic attestation of selected
round-integrity claims. Phase 1 ships **proof-ready** so Phase 2 needs no rework; the proof-ready
disciplines (deterministic aggregation,
...
| Surface | Current runtime (2026-07-24) | Intended Phase 1 (v0.1–v1.0) | Phase 2 (Stage D, post-v1.0) |
|---|---|---|---|
| Raw-data confidentiality | Residency guard is live; process/transport boundary remains simulated | Residency enforcement (`INV-RESIDENCY`, fail-closed) | Same |
| Individual update confidentiality | **Blocked:** coordinator receives unmasked $\Delta_c$ | Secure aggregation hides $\Delta_c$ (honest-but-curious aggregator, collusion bound) | Same; aggregation also attestable |
| Per-participant-round leakage bound | **Blocked:** noise is replayable from shared root seed; accounting is one fresh post-commit step | Participant-secret DP with cumulative $(\varepsilon,\delta)$ enforcement | Same |
| Aggregation arithmetic correctness | Determinism self-check; coordinator honesty assumed | **Assumed** (honest coordinator) | **Proven** (STARK) |
| Provenance / data origin | Tamper-evident Merkle binding | Tamper-evident (Merkle binding) | **Proven** (cryptographic claim) |
| Frame alignment correctness | Publicly recomputable | Publicly recomputable | Same |
| Inner-step faithfulness | **Assumed** | **Assumed** | **Attested** (TEE) — still an assumption on TEE hardware |
| Data quality / gradient honesty | **Not guaranteed** (out of scope) | **Not guaranteed** (out of scope) | **Not guaranteed** (out of scope) |

The honest reading: the current runtime enforces raw-data residency guards and exercises clip/noise,
accountant, and aggregation primitives, but it does not yet provide individual-update confidentiality or
claim-grade cumulative DP. Intended Phase 1 requires secure aggregation on the optimizer path,
participant-secret CSPRNG entropy, and persistent pre-release accounting. Phase 2 can then replace
aggregation and inner-step honesty assumptions with a proof and an attestation. Data quality and
gradient honesty remain out of scope in every phase.
