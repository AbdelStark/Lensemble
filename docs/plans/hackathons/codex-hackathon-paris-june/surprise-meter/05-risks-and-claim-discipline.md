...
| R5 | WebGPU absent/unstable on projector machine | Med | High | ⚠️ There is **no clean WebGPU→WASM degradation today**: `lewm_runtime.mjs:85-92` creates sessions once with `executionProviders:["webgpu","wasm"]` and throws `session-create-failed` on any failure (no per-provider retry). Mitigation: (1) thread a **`?ep=wasm` URL param → `preferredProviders`** override through `app.mjs`→`loadLewmRuntime`; (2) add a per-provider retry loop in `lewm_runtime.mjs`; (3) the meter itself is SVG (`charts.mjs`, no WebGPU); clip rung D last. | SM-3 |
