...
| C12 | "WASM fallback already there"; offline validation | **Mitigated in implementation:** the surprise-meter supports `?engine=live&ep=wasm`, and `loadLewmRuntime` retries provider sets before failing. `onnxruntime` is still not a baseline repo dependency, so offline bakes use `uv run --with onnxruntime …` (Python) or a node project with `onnxruntime-node` + an injected `file://` fetchFn. | `lewm_runtime.mjs`; `web/surprise-meter/app.mjs` |
