...
- `federatedRelativeImprovement` and `federatedSeedMeanImprovement` are **read from** the cited existing evidence files, never recomputed here (Decision D3). The producer asserts the source files exist and the values match within 1e-6.
