...
| **D9** | **New evidence schema `lewm-manifold/1`**, producer `scripts/lewm_manifold_check.py` → `docs/evidence/lewm_tworooms_manifold.json`, with explicit `nonClaims`, validated by `tests/ml/test_lewm_manifold.py`. | Matches repo evidence discipline (AGENTS.md §16-17); not a `demo-evidence/1` real-mode bundle, so no `audit_real_lewm_evidence` requirement, but `nonClaims` is mandatory. |
...
| Block | Work |
|-------|------|
| Pre-hack (Wed eve) | Bake a **v0 fallback** `manifold.json` from current checkpoint so a viewer exists before any live work. Vendor Three.js. |
