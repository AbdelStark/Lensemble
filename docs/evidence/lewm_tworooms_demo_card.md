...
adapter experiment** ([#314](https://github.com/AbdelStark/Lensemble/issues/314); contract in
...
> Lensemble demonstrates a **Tapestry-like browser-local federated adaptation run**: federated
> adapter continuation on a frozen checkpoint. Each browser trains only a 12,512-parameter
> (0.069%) residual adapter on the frozen predictor output, then submits its clipped adapter delta
> and derived summaries to one local coordinator. This is not full-model or browser world-model
> training.
...
| Browser inference | The browser downloads hash-pinned, weight-bearing ONNX graphs (encoder+projector, action encoder, predictor+pred_proj) exported deterministically from the pinned checkpoint, with ImageNet pixel normalization and the expert-dataset action z-score baked in; PyTorch-vs-onnxruntime parity ≤ 1e-4 | [export manifest](lewm_tworooms_browser_export_manifest.json), [action stats](lewm_tworooms_action_stats.json) |
...
| Browser-local training | A bounded zero-init residual adapter (12,512 parameters, 0.069% of the checkpoint) on the frozen predictor output, trained with manual-gradient Adam in the browser; SIGReg Epps–Pulley diagnostics ported exactly from `lensemble.model.sigreg` (torch parity ≤ 2e-7 rel); one-browser overfit on real latents: pred loss 0.0559 → 0.00061 | [adapter overfit](lewm_tworooms_adapter_overfit.json) |
| Federation | Each `lewm-adapter-delta/1` clipped adapter delta is visible to and validated by a single local coordinator (schema, byte budget, shapes, norms, checkpoint/export-hash freshness, replay, fabricated-metric rejection), then aggregated as a deterministic per-round mean over a shared deterministic adapter init. This path has no secure aggregation or differential privacy | `tests/ml/test_lewm_federation.py` |
| Before/after probe (system-composed) | The system-composed probe is the headline: real node-trained adapter deltas flow through `FederatedDemoService.submit_update` (real validation) and `_close_round_lewm` (real deterministic-mean aggregation + hash-chained revisions), and the probe scores the server-produced final `modelRevisionId`, not an offline-recomputed mean. Held-out MSE improved 0.0604 → 0.0530 (**+12.3%**, verdict `improved`, 0 claim-audit violations) on the seed-`20260612` draw. `scripts/lewm_probe_check.py` is only an offline math cross-check | [system-composed probe](lewm_tworooms_system_probe.json); offline math cross-check [federated probe](lewm_tworooms_probe_check.json) |
...
- Raw rollouts, labels, actions, intermediate activations, and latents remain in each participant
  browser in this demo protocol. The evidence audit rejects raw-data-like fields fail-closed.
- The browser downloads hash-pinned, weight-bearing ONNX graphs. The residency boundary applies
  to participant-generated training material, not to secrecy of the frozen model weights.
- The upload boundary contains clipped adapter deltas and derived summaries. A single local
  coordinator sees each individual delta before computing the deterministic mean; the deltas are
  neither anonymous nor hidden from that coordinator.
- Secure aggregation (RFC-0011) and differential privacy (RFC-0012) exist elsewhere in Lensemble,
  but there is **no secure aggregation or differential privacy in this demo path**. The demo makes
  no cryptographic privacy claim for uploaded adapter deltas.
...
- a privacy guarantee for individual adapter deltas or a model-secrecy mechanism.
