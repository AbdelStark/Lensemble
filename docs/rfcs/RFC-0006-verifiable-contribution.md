...
2. **Did a participant compute its update from the data it claims?** Without binding, a participant can submit an update derived from data it never committed to, breaking the claimed provenance link.
...
- **Replacing secure aggregation or DP.** Verifiable contribution is orthogonal to confidentiality:
  target secure aggregation ([RFC-0011](RFC-0011-secure-aggregation.md)) hides individual updates and
  effective DP ([RFC-0012](RFC-0012-differential-privacy.md)) bounds leakage; a
  future proof integration would establish that protected updates were
  aggregated correctly. The current plaintext/replay path supplies neither
  protection.
...
|---|---|---|---|---|
| **Phase 1 target** | honest-but-curious | honest, may drop out | raw-data residency plus secure aggregation + effective DP for released updates ([RFC-0011](RFC-0011-secure-aggregation.md), [RFC-0012](RFC-0012-differential-privacy.md)); current runtime has residency only, with plaintext updates and replayable noise | provenance commitments ([RFC-0014](RFC-0014-provenance-commitments.md)) give tamper-*evidence*; aggregation integrity is *assumed* |
| **Phase 2** | may be malicious; misbehavior **detectable** | may misreport; binding makes it **detectable** | unchanged | aggregation correctness *proven* (STARK); provenance binding *proven* (Merkle); committed-model alignment diagnostic *publicly recomputable*; participant-side backstop *attested/proven or trusted-boundary executed*; inner step *TEE-attested* |
...
| **Committed global model's frame diagnostic correct** | **Public recomputation** — a deterministic function of the public probe + committed weights | **Free.** Anyone recomputes the committed model's $Q^\star$/residual. This does not verify a participant-specific pre-release transform. Residual assumption: none beyond `INV-PROBE-PIN` and `INV-CHECKPOINT-HASH`. |
| **Participant-specific backstop applied before release** | Participant attestation/proof, or execution inside the trusted/MPC aggregation boundary (`INV-ALIGN-BEFORE-RELEASE`) | Not free: the action needs raw local weights that a secure sum intentionally hides. Until one mechanism is integrated, participant execution is assumed. |
...
The claims compose into a single statement per round (item 5): *the global model at round $t{+}1$ is
the deterministic aggregate of $C$ updates, each bound to a committed dataset root and produced inside
its declared attested/trusted boundary; the committed model's frame diagnostic is publicly
recomputable.* The strength of that statement is exactly the weakest link, which is why
participant-side alignment is not collapsed into the public diagnostic.
...
a weighted average of committed deltas followed by a fixed Nesterov update. Were participants in
mutually-rotated coordinate frames (the $O(d)$ gauge of
[RFC-0002 §2](RFC-0002-gauge-and-aggregation.md)), averaging requires a participant-specific
orthogonal alignment $Q_c$ **before** clip/noise/quantization/masking. A sum-only circuit cannot apply
that transform afterward. If the Layer-3 backstop binds, its participant-local execution must be
attested/proven or performed inside the trusted/MPC boundary and bound to the submitted aligned delta.
The outer-step STARK may still prove only the near-linear sum + Nesterov step; it then proves correct
aggregation of submitted aligned releases, not that participants aligned honestly. Keeping the
backstop quiet via warm-start + anchor therefore keeps both computation and the integrity statement
cheap. **Closing the gauge keeps the proof cheap without pretending the hidden participant action is
public.**
...
The free Phase-1 surface is public recomputation of the **committed global model's** frame diagnostic.
It is a deterministic, side-effect-free function of public data and committed weights, exposed under
`lensemble.verify`. It deliberately does not verify whether each participant applied the Layer-3
backstop before release:
...
|---|---|---|---|---|---|
| **Phase 1** (`v0.1`–`v1.0`) | Z (deterministic + recomputable) | Z (tamper-evident) | Z for participant action; P only for committed-model diagnostic | Z | "Honest if the coordinator and participants are honest; the committed-model diagnostic is detectable, but participant pre-release alignment is assumed." |
| **2a** | **P** (STARK) | Z | Z/P as above | Z | "Aggregation provably correct over submitted deltas; participant alignment, provenance, and inner step assumed." |
| **2b** | P | **P** (Merkle binding) | Z/P as above | Z | "Each update provably bound to a committed dataset root; aggregation provably correct; participant alignment and inner step assumed." |
| **2c** | P | P | **A** when included in the attested participant/TEE boundary; P for diagnostic | **A** (TEE) | "Inner step and pre-release alignment attested by the declared boundary; aggregation and binding proven; committed-model diagnostic recomputable." |
| **2d** *(research)* | P | P | toward **P** for participant action | toward **P** | "Push participant computation from attested toward proven as prover capability improves." |
...
| Recomputed committed-model diagnostic ≠ published claim | `recompute_alignment(..., expected=...)` returns `matches_expected=False` | none (returns a record) | CLI exits non-zero; the diagnostic claim is rejected; this says nothing about participant-side backstop execution |
| Participant-specific backstop cannot be verified across a secure-sum-only boundary | information-flow preflight (`INV-ALIGN-BEFORE-RELEASE`) | `ConfigError` in the current coordinator harness | require participant attestation/proof or execute alignment inside the trusted/MPC boundary |
...
*Why rejected:* proving a full 1.2 B training step in zero knowledge is infeasible today (Non-Goals).
The layered model puts the cheapest sufficient mechanism behind each claim — free recomputation for
the committed-model diagnostic, a small STARK for near-linear aggregation, a Merkle binding for
provenance, and a TEE/attestation for the heavy data-touching inner step **including any participant
alignment** — and composes them into one honest statement (item 5).
...
- **`v0.1` (Stage A) and `v0.2` (Stage B):** the five proof-ready requirements (deterministic aggregation, committed model versions, Merkle roots, pinned probe, reproducible outer step) ship and are tested. The free committed-model diagnostic (`recompute_alignment`, `lensemble verify recompute`) ships. `lensemble verify prove` is a stub that raises `NotImplementedError`.
- **`v0.3` (Stage C):** the disciplines operate over a real network boundary with real secure aggregation and DP; participant alignment moves before the release pipeline (`INV-ALIGN-BEFORE-RELEASE`), and the `ContributionLedger` ([RFC-0014](RFC-0014-provenance-commitments.md)) is populated. No proofs yet; participant execution remains assumed.
...
- **Public committed-model alignment diagnostic.** `recompute_alignment` reproduces the committed model's diagnostic from the public probe + committed weights to within fp32/fp64 tolerance; assert `matches_expected=True` for an honest claim and `False` for a perturbed claim. This test must not be described as verification of participant-side backstop execution.
- **Alignment information-flow guard.** A coordinator-side participant-specific backstop rejects DP-enabled, quantized, or sum-only-backend configurations and release-transformed updates (`INV-ALIGN-BEFORE-RELEASE`).
...
DECIDED DIRECTION: When the Layer-3 Procrustes backstop
([RFC-0002 §5](RFC-0002-gauge-and-aggregation.md)) binds, it must execute participant-side before
clip/noise/quantization/masking or inside a trusted/MPC boundary. A secure sum cannot make that
participant-specific action publicly recomputable. The remaining Stage-D choice is whether to attest
the action with the inner step or prove it/bind it in a larger circuit; the near-linear outer STARK may
continue to prove only aggregation of the already-aligned releases.

RISK: The Phase-2 design rests on the aggregation circuit staying near-linear (item 2.1), which in turn rests on the frame anchor keeping the Layer-3 backstop quiet. If the gauge proves hard to control at video-WM scale ([RFC-0002 §7](RFC-0002-gauge-and-aggregation.md) and Open Questions), the aggregation circuit grows and the low-cost proof target weakens. Resolution plan: the Stage-B frame-drift sweep quantifies backstop frequency before any Stage-D proof work begins; if drift is severe, the Fork-A fallback ([RFC-0002 — Fork A fallback](RFC-0002-gauge-and-aggregation.md#fork-a-fallback)) dissolves the gauge entirely and the aggregation is linear (a frozen-encoder predictor average), preserving the simpler proof target while narrowing the research scope to predictor federation.
...
- [RFC-0004 — Data, Sovereignty & Provenance](RFC-0004-data-provenance.md): the public probe (§3), provenance commitments and the origin-not-quality boundary (§4), provenance records (§5).
