# surprise-meter — Milestone-1 stage fallback

The live **surprise-meter** for the Codex-Paris demo (issue
[#338](https://github.com/AbdelStark/Lensemble/issues/338), Milestone 1). The page runs as a
stage fallback with zero ONNX/network dependency: it loads the generated
`data/result_card.json` and `data/surprise_trajectory.json`, then renders the certified held-out
numbers beside a recorded TwoRooms surprise trajectory.
...
the warm-up behavior and scalar MSE parity without ONNX Runtime.
...
`baselineMse 0.0604 → adaptedMse 0.0530` (= +12.3%); the recorded trajectory is a fallback
visualization, not a second benchmark.
